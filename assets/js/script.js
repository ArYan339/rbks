...
document.addEventListener('visibilitychange',
    function () {
        if (document.visibilityState === "visible") {
            document.title = "Portfolio | Rambachan Kushwaha";
...
(function() {
    ...
    s1.onerror = function() {
        ...
        // You can add custom error handling here, such as displaying a message to the user
    };
    s0.parentNode.insertBefore(s1, s0);

    // Check connection status periodically
    setInterval(function() {
        if (Tawk_API.getStatus) {
            var status = Tawk_API.getStatus();
            if (status === 'offline') {
                console.warn("Tawk.to is offline");
                // You can add custom handling for offline status here
            } else if (status === 'away') {
                console.log("Tawk.to status: Away");
